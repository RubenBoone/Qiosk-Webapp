...
export class TagFormComponent implements OnInit, OnDestroy {
  ...
  constructor(private router: Router, private tagService: TagService) {
    ...
    if (this.tagID != null && this.tagID > 0) {
      this.tag$ = this.tagService.getTag(this.tagID).subscribe(result => this.tag = result);
    }
  ...
  onSubmit() {
    ...
    if (this.isAdd) {
      this.postTag$ = this.tagService.postTag(this.tag).subscribe(result => {
                //all went well
    ...
    if (this.isEdit) {
      this.putTag$ = this.tagService.putTag(this.tagID, this.tag).subscribe(result => {
                //all went well
                this.router.navigateByUrl("/admin/tags");
              },
              error => {
